...
import { FaUserShield, FaSave, FaArrowLeft, FaCheck, FaInfoCircle, FaShieldAlt } from 'react-icons/fa';
...
export default function Edit({ role, permissions }) {
    ...
    const toggleGroup = (moduleName, groupPermissions) => {
        ...
        if (allSelected) {
            ...
            const currentIds = new Set(data.permissions);
            groupIds.forEach(id => currentIds.add(id));
            setData('permissions', Array.from(currentIds));
        }
    };
                ...
                <form onSubmit={handleSubmit} className="w-full mx-auto px-4 space-y-4">
                    ...
                    <div className="bg-white rounded-lg shadow-sm border border-slate-200 p-3 flex justify-between items-center sticky top-4 z-20">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h1 className="text-xs font-normal text-slate-900 uppercase tracking-normal">Update: {role.name}</h1>
                                <p className="text-[9px] text-slate-400 font-normal uppercase tracking-normal">Update role details and permissions</p>
                            </div>
                        </div>
                        <div className="flex items-center gap-2">
                            ...
                            >
                                {processing ? 'Saving...' : <><FaSave /> Save Changes</>}
                            </button>
                        </div>
                    </div>
                        ...
                        <div className="lg:col-span-8 space-y-4">
                            ...
                            <div className="bg-white rounded-lg shadow-sm border border-slate-200 overflow-hidden">
                                ...
                                <div className="p-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <div className="space-y-1">
                                        <label className="text-[10px] font-normal text-slate-400 uppercase tracking-normal ml-1">System Slug (Restricted)</label>
                                        <input
                                            type="text"
                                            value={data.slug}
                                    ...
                                    {Object.keys(groupedPermissions).map((module) => (
                                        <div key={module} className="space-y-2">
                                            <div className="flex items-center justify-between px-3 py-2 bg-slate-50 border-l-2 border-primary rounded-r-lg mb-2">
                                                ...
                                                <button
                                                    type="button"
                                                    onClick={() => toggleGroup(module, groupedPermissions[module])}
                                                    className="text-[7px] font-normal text-slate-400 hover:text-primary uppercase tracking-normal transition-colors"
                                                >
                                                    {groupedPermissions[module].every(p => data.permissions.includes(p.id)) ? 'Deselect All' : 'Select All'}
                                                </button>
                                            </div>
                            ...
                            <div className="bg-white rounded-lg shadow-sm border border-slate-200 overflow-hidden">
                                ...
                                <div className="p-4 space-y-4">
                                    <div className="flex justify-between items-center py-2 border-b border-slate-50">
                                        <span className="text-[9px] font-normal text-slate-400 uppercase">Rights Assigned</span>
                                        <span className="text-[10px] font-normal text-slate-900 tabular-nums">{data.permissions.length} Active</span>
                                    </div>
                                    <div className="flex justify-between items-center py-2 border-b border-slate-50">
                                    ...
                                    <div className="p-3 bg-slate-50 rounded-lg border border-slate-100">
                                        <p className="text-[8px] text-slate-400 font-normal leading-relaxed uppercase">
                                            Updating role configurations affects all assigned users immediately. Ensure system-wide impact is considered before committing changes.
                                        </p>
                                    </div>
                                </div>
                            </div>
