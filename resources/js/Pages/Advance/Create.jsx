...
import { FaMoneyBillWave, FaCalendarAlt, FaUser, FaInfoCircle, FaSave, FaArrowLeft, FaCheck, FaShieldAlt, FaClock, FaPlus, FaChevronDown, FaHistory, FaChartPie } from 'react-icons/fa';

export default function Create({ employees, userRole = 'employee' }) {
    const { appSettings } = usePage().props;
    const currency = appSettings?.currency || 'QAR';
    const currency_symbol = appSettings?.currency_symbol || 'QAR';

    const { data, setData, post, processing, errors } = useForm({
        employee_id: userRole === 'employee' && employees.length > 0 ? employees[0].id : '',
        amount: '',
            ...
            <div className="p-4 sm:p-6 space-y-4">
                ...
                <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-white p-4 rounded-lg border border-slate-200 shadow-sm relative overflow-hidden">
                    ...
                    <div className="flex items-center gap-4 relative z-10">
                        ...
                        <div>
                            <h2 className="text-xl font-normal text-slate-900 tracking-normal leading-none mb-2 uppercase">Request Advance</h2>
                            <p className="text-[10px] font-normal text-slate-400 uppercase flex items-center gap-2">
                                <span className="w-1.5 h-1.5 bg-primary rounded-full animate-pulse"></span>
                                {userRole === 'employee' ? 'Employee Request' : 'Admin Entry'}
                            </p>
                        </div>
                    </div>
                ...
                <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-4 items-start">
                    ...
                    <div className="space-y-4 sticky top-6">
                        <div className="bg-slate-900 rounded-lg p-5 text-white shadow-2xl shadow-slate-200 relative overflow-hidden group">
                            ...
                            <h3 className="text-[10px] font-normal uppercase mb-6 text-white flex items-center gap-2">
                            ...
                            <div className="space-y-6 relative z-10">
                                <div>
                                    <p className="text-[10px] font-normal text-white uppercase mb-1">Advance Amount</p>
                                ...
                                <div className="space-y-3 pt-6 border-t border-white/10">
                                    <div className="flex justify-between items-center">
                                        <span className="text-[10px] font-normal text-white uppercase">Request Date</span>
                                        <span className="text-[10px] font-normal text-white uppercase">{formatDate(data.request_date)}</span>
                                    </div>
                                    <div className="flex justify-between items-center">
                                        <span className="text-[10px] font-normal text-white uppercase">Expected Repayment</span>
                                        <span className="text-[10px] font-normal text-white uppercase">{formatDate(data.repayment_date)}</span>
                                    </div>
                                </div>
                        ...
                        <div className="bg-white rounded-lg p-4 border border-slate-200 shadow-sm">
                            <h3 className="text-[10px] font-normal text-slate-900 uppercase mb-4 flex items-center gap-2">
                                <FaShieldAlt className="text-primary" /> Advance Terms
                            </h3>
                            <div className="space-y-3">
                                {[
                                    'Advance repayment is deducted in next cycle',
                                    'Requests are subject to administrative review',
                                    'Employees can only have one active advance'
                                ].map((text, i) => (
                                    <div key={i} className="flex gap-3">
                                        <FaCheck className="text-emerald-500 shrink-0 mt-1" size={10} />
                        ...
                        <div className="bg-slate-50 p-4 rounded-lg border border-slate-100 flex gap-4">
                            <FaClock className="text-primary shrink-0 mt-1" size={14} />
                            <p className="text-[9px] font-normal text-slate-500 uppercase leading-relaxed">
                                TYPICAL REVIEW TIME: 24H FROM SUBMISSION.
                            </p>
                        </div>
                    </div>
                </form>
