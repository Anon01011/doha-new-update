...
export default function Slip({ salaryPosting, loanInstallments = [], advances = [], overtimeDetails = null }) {
    ...
    const handlePrint = () => {
        if (salaryPosting.status?.toLowerCase() !== 'approved') {
            setWarningModal({
                show: true,
                message: 'This salary slip is currently in ' + (salaryPosting.status || 'draft') + ' status. You can only print officially approved salary slips.'
            });
            return;
        }
        window.print();
    ...
    const handleDownload = () => {
        if (salaryPosting.status?.toLowerCase() !== 'approved') {
            setWarningModal({
                show: true,
                message: 'This salary slip is currently in ' + (salaryPosting.status || 'draft') + ' status. You can only download officially approved salary slips.'
            });
            return;
        }
    ...
    const getCurrencyWord = (curr) => {
        if (curr === 'INR') return 'Rupees';
        if (curr === 'QAR') return 'Qatari Riyals';
        if (curr === 'AED') return 'Dirhams';
        if (curr === 'USD') return 'Dollars';
        return curr;
    };
    const getSubCurrencyWord = (curr) => {
        if (curr === 'INR') return 'Paise';
        if (curr === 'QAR' || curr === 'AED') return 'Dirhams';
        if (curr === 'USD') return 'Cents';
        return 'Cents';
    };
    const wordsStr = `${numberToWords(netInt)} ${getCurrencyWord(currency)} ${netDec > 0 ? `and ${numberToWords(netDec)} ${getSubCurrencyWord(currency)} ` : ''}Only`;
        ...
        <AuthenticatedLayout>
            ...
            <div className="min-h-screen bg-[#F8FAFC] p-3 sm:p-6 lg:p-8 print:p-0 print:bg-white">
                ...
                <div className="overflow-x-auto pb-4 no-scrollbar">
                    <div id="printable-slip" className="min-w-[700px] md:min-w-0 max-w-[800px] mx-auto bg-white border border-gray-300 shadow-xl print:shadow-none print:border-none print-container">
